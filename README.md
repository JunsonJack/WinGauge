...
| 钉住 + 拖动 | ✅ | 原生 `startDragging`，丝滑、**支持跨显示器**；钉住后失焦不收起 |
| **桌面胶囊** | ✅ | 收起为 320×48 横条，实时 CPU / 内存 / 网速 + 健康点；失焦不收起，双击或按钮展开 |
| 设备头 | ✅ | 主机名 / 系统版本 / CPU / 内存总量 / 运行时长 |
| 健康度总分 | ✅ | 100 起扣，可展开扣分明细 |
| CPU 卡片 | ✅ | 总使用率、**温度徽标**、峰值、每核柱状图、负载档位、60s 趋势 |
| 风扇卡片 | ✅ | 联想 EC 命中时显示各风扇 RPM；其他机型隐藏 |
| 厂商温度/风扇 | ✅ | `LENOVO_FAN_METHOD` 扫描 SensorID/FanID；通用 ACPI 假数据永不展示 |
| 内存卡片 | ✅ | 已用/总量、已提交内存（Windows 语义，不出现「交换」歧义）、趋势 |
| 网络卡片 | ✅ | 主接口下行/上行速率 + 接口友好名、趋势 |
| 磁盘卡片 | ✅ | 系统盘用量、剩余空间 |
| 设置页 | ✅ | 明暗主题、透明度、自启、**AtomGit 手动检查更新**、退出 |
...
| **W4b** 厂商传感器 | 联想温度/风扇 + ID 扫描 | ✅ v0.1.0 |
